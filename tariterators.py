...
def group_by_keys(
    ...
    for filesample in data:
        try:
            ...
            # jinwooahn
            # suffix가 txt일경우 value를 sep token으로 나눠서 random 으로 설정하게끔 하면 됨
            if suffix == "txt":
              sep_token = "<[!tab]>"
              value_candidates = value.split(sep_token)
              value = random.choice(value_candidates).strip()
            if trace:
                print(
                    prefix,
                    suffix,
            ...
            if suffixes is None or suffix in suffixes:
                current_sample[suffix] = value #여기 value 를 sep token 기준으로 slice한 뒤 random module로 test해보면 될 것 같음
